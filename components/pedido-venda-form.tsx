...
export default function PedidoVendaForm() {
  ...
  const buscarProdutos = async (search: string) => {
    ...
    try {
      // Usar endpoint otimizado com cache
      const response = await fetch(`/api/sankhya/produtos/search?q=${encodeURIComponent(search)}&limit=20`)
      const data = await response.json()
      
      if (data.produtos && data.produtos.length > 0) {
        // Buscar preços em lote
        const codigos = data.produtos.map((p: any) => p.CODPROD).join(',')
        try {
          const precoResponse = await fetch(`/api/sankhya/produtos/batch-info?codigos=${codigos}`)
          const precoData = await precoResponse.json()
          
          const produtosComPreco = data.produtos.map((produto: any) => {
            const infoPreco = precoData.produtos?.find((p: any) => p.CODPROD === produto.CODPROD)
            return {
              ...produto,
              VLRCOMERC: infoPreco?.VLRCOMERC || produto.VLRCOMERC || '0',
              VLRVENDA: infoPreco?.VLRCOMERC || produto.VLRVENDA || '0'
            }
          })
          
          setProdutos(produtosComPreco)
        } catch (error) {
          console.warn('Erro ao buscar preços em lote:', error)
    ...
    <div className="space-y-6">
      ...
      <Dialog open={showProdutoModal} onOpenChange={setShowProdutoModal}>
        <DialogContent className="max-w-2xl max-h-[80vh] border-green-200">
          <DialogHeader className="border-b border-green-100 pb-4">
            <DialogTitle className="text-green-800">Buscar Produto</DialogTitle>
          </DialogHeader>
            ...
            <div className="max-h-96 overflow-y-auto space-y-2">
              {isLoadingProducts ? (
                ...
                produtos.map((produto) => (
                  <Card
                    key={produto.CODPROD}
                    className="border-green-100 hover:bg-green-50 cursor-pointer transition-colors"
      ...
      <EstoqueModal
        ...
        onClose={() => {
          setShowEstoqueModal(false)
          if (!itemAtual.CODPROD) {
            setShowItemModal(true) // Reabrir modal de item se não confirmou
          }
        }}
