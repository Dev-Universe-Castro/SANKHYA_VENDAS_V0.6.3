...
export const authService = {
  ...
  async login(email: string, password: string): Promise<User | null> {
    try {
      ...
      if (user) {
        currentUser = user;
        // Store in localStorage for persistence (cookie já é definido no servidor)
        if (typeof window !== "undefined") {
          localStorage.setItem("currentUser", JSON.stringify(user));
          // Também definir cookie no cliente para acesso imediato
          const userCookie = JSON.stringify(user);
          document.cookie = `user=${userCookie}; path=/; max-age=${60 * 60 * 24 * 7}`;
          console.log('🍪 Cookie de usuário definido:', { id: user.id, name: user.name, codVendedor: user.codVendedor });
        }
        return user;
      }
      return null;
